...
template <typename G1, typename G2> class ProductGroup : public Group {
public:
...
public:
  ProductGroup(const G1 &_g1, const G2 &_g2) : g1(_g1), g2(_g2) {}

  static ProductGroupElement<ELEMENT1, ELEMENT2> dummy_element() {
    return ProductGroupElement<ELEMENT1, ELEMENT2>(G1::dummy_element(),
                                                   G2::dummy_element());
  }

  static ProductGroupIrrep<IRREP1, IRREP2> dummy_irrep() {
    return ProductGroupIrrep<IRREP1, IRREP2>(G1::dummy_irrep(),
                                             G2::dummy_irrep());
  }

public:
  int size() const { return g1.size() * g2.size(); }

  ProductGroupElement<decltype(g1.identity()), decltype(g2.identity())>
  identity() const {
    return gpair(g1.identity(), g2.identity());
  }

  ProductGroupElement<decltype(g1.identity()), decltype(g2.identity())>
  element(const int i) const {
    return gpair(g1.element(i / g2.size()), g2.element(i % g2.size()));
  }

public:
  int n_irreps() const { return g1.n_irreps() * g2.n_irreps(); }
...
template <typename G1, typename G2,
          typename = typename std::enable_if<std::is_base_of<Group, G1>::value,
                                             G1>::type>
