...
public: // Constructors
  Permutation(const int _n) : n(_n) { p = new int[n]; }

  Permutation(const int _n, const cnine::fill_raw &dummy) : n(_n) {
    p = new int[n];
  }

  Permutation(const int _n, const cnine::fill_identity &dummy) : n(_n) {
    p = new int[n];
    for (int i = 0; i < n; i++)
      p[i] = i + 1;
  }
...
public
  ...
  int operator()(const int i) const { return p[i - 1]; }

  int operator[](const int i) const { return p[i]; }

  int &operator[](const int i) { return p[i]; }

  void set_value(const int i, const int j) { p[i - 1] = j; }
...
public:
  ...
  Permutation inv() const { return inverse(); }

  bool check_valid() const {
    vector<bool> a(n, false);
    for (int i = 0; i < n; i++) {
      if (p[i] < 1 || p[i] > n || a[p[i] - 1])
        return false;
      a[p[i] - 1] = true;
    }
    return true;
