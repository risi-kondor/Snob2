...
public:
  SnIrrepObj(const IntegerPartition &_lambda) : lambda(_lambda) {
...
public
  ...
  void apply_left(const cnine::Rtensor3_view &A,
                  const ContiguousCycle &cyc) const {
    if (cyc.a < cyc.b) {
      for (int i = cyc.b - 1; i >= cyc.a; i--)
        apply_left(A, i);
    } else {
  ...
  void apply_left(const cnine::Rtensor3_view &T, const int tau) const {
    ...
    cnine::ScaleSomeSlicesFn()(
        T, YORdiagIndices->view2().slice0(tau - 1).block(0, ndiags),
        YORdiagCoeffs->view2().slice0(tau - 1).block(0, ndiags));
    cnine::GivensSomeSlicesFn()(T,
                                YORgivensIndices->view3().slice0(tau - 1).block(
                                    0, 0, (d - ndiags) / 2, 2),
                                YORgivensCoeffs->view3().slice0(tau - 1).block(
                                    0, 0, (d - ndiags) / 2, 2));
...
public
  ...
  void apply_left(rtensor &A, const ContiguousCycle &cyc, const int beg,
                  const int end) const {
    for (int i = cyc.b - 1; i >= cyc.a; i--)
      apply_left(A, i, beg, end);
  }

  void apply_left_inv(rtensor &A, const ContiguousCycle &cyc, const int beg,
                      const int end) const {
    for (int i = cyc.a; i < cyc.b; i++)
      apply_left(A, i, beg, end);
  }
...
public: // ---- apply_left to multiple blocks
        // --------------------------------------------------------------
  void apply_left(rtensor &A, const SnElement sigma, const int beg,
                  const int end, const int stride) const {
    SNOB2_ASSERT(sigma.getn() == n, "Permutation wrong size");
    vector<int> shifts(n);
    for (int i = n; i > 0; i--) {
      int a = sigma(i);
...
private
  ...
  void computeYOR() const {
    ...
    for (int i = 0; i < d; i++) { // for each tableau...
      for (int tau = 1; tau < n;
           tau++) { // for each transposition (tau,tau+1)...
        ...
        if ((delta == 1) || (delta == -1)) {
          ...
          // cout<<tableau.shape()<<" "<<i<<" "<<tau<<" "<<YORt[ix]<<"
          // "<<delta<<endl;
          YOR1[ix] = 1.0 / delta;
  ...
  void computeYORtensors() const {
    ...
    YORgivensIndices =
        new itensor(cnine::Gdims({n - 1, d / 2 + 1, 2}), cnine::fill_zero());
    YORgivensCoeffs =
        new rtensor(cnine::Gdims({n - 1, d / 2 + 1, 2}), cnine::fill_zero());
    ...
    for (int tau = 1; tau < n; tau++) { // for each transposition (tau,tau+1)...
      ...
      for (int i = 0; i < d; i++) { // for each tableau...
        ...
        if ((delta == 1) || (delta == -1)) {
          ...
          YORgivensCoeffs->set(tau - 1, i - ix, 1,
                               sqrt(1.0 - 1.0 / (delta * delta)));
          done[j] = true;
        }
      }
