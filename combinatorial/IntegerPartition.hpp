...
public:
  IntegerPartition() : k(0) { p = nullptr; }
...
private:
  int factorial(int x) const { return x == 0 ? 1 : x *= factorial(x - 1); }

public: // Access
  int height() const { return k; }
  ...
  int &operator[](const int r) { return p[r]; }

  int operator[](const int r) const { return p[r]; }

  int &operator()(const int r) { return p[r]; }

  int operator()(const int r) const { return p[r]; }

  void set(const int r, const int x) { p[r] = x; }
...
public:
  ...
  bool operator<(const IntegerPartition &y) const {
    SNOB2_ASSERT(getn() == y.getn(),
                 "Comparing partition of n with partition of m<>n.");
...
public:
  vector<IntegerPartition> parents() const {
    ...
    for_each_sub([&](const IntegerPartition &mu) { R.push_back(mu); });
    return R;
  }
...
namespace std {
template <> struct hash<Snob2::IntegerPartition> {
public:
  size_t operator()(const Snob2::IntegerPartition &x) const {
    size_t r = 1;
    for (int i = 0; i < x.k; i++)
      r = (r << 1) ^ hash<int>()(x.p[i]);
    return r;
  }
};
} // namespace std
...
public:
  size_t operator()(const pair<Snob2::IntegerPartition, Snob2::IntegerPartition>
                        &lambdas) const {
...
namespace std {
template <>
struct hash<std::tuple<Snob2::IntegerPartition, Snob2::IntegerPartition,
                       Snob2::IntegerPartition>> {
public:
  size_t
  operator()(const tuple<Snob2::IntegerPartition, Snob2::IntegerPartition,
                         Snob2::IntegerPartition> &lambdas) const {
