...
public
  ...
  int index(const IntegerPartition &lambda) const {
    for (int i = 0; i < parts.size(); i++)
      if (parts[i]->irrep->lambda == lambda)
        return i;
    return -1;
  }
...
public
  ...
  void accumulate_up(SnType &offs, const SnVec &sub, const SnType &tau,
                     const string indent = "") {
    // cout<<indent<<"Accumulate up SnVec of type "<<sub.get_type()<<" into
    // "<<get_type()<<" with mask "<<tau<<endl;
    SnType sub_offs;
    for (auto &p : tau) {
      ...
      lamb.foreach_sub([&](const IntegerPartition &mu) {
        ...
        target.block2(roffs, coffs, nrows, ncols)
            .add(source.block2(0, sub_offs[mu], nrows, ncols));
        sub_offs[mu] += ncols;
        roffs += nrows;
      });
      offs[lamb] += ncols;
  ...
  SnVec static up(const SnType &tau, const SnVec &v) {
    ...
    for (auto &p : tau) {
      ...
      p.first.foreach_sub([&](const IntegerPartition &mu) {
        int ix = v.index(mu);
        assert(ix >= 0);
        dest.add_to_block_block(voffs, 0, *v.parts[ix], 0, offs[ix],
                                v.parts[ix]->dims(0), m);
...
// FILLTYPE>::type> SnVec(const Sn& G,const FILLTYPE& fill, const int _dev=0){
...
template<typename FILLTYPE, typename = typename
std::enable_if<std::is_base_of<cnine::fill_pattern, FILLTYPE>::value,
FILLTYPE>::type> SnVec(const SnRepresentationObj* _repr, const FILLTYPE& fill,
const int _dev=0)//: repr(_repr)
{
