...
public:
  SnPartB() : rtensor(), irrep(nullptr) {}

  template <typename FILLTYPE,
            typename = typename std::enable_if<
                std::is_base_of<cnine::fill_pattern, FILLTYPE>::value,
                FILLTYPE>::type>
  SnPartB(const initializer_list<int> list, const int n, const FILLTYPE &fill,
          const int _dev = 0)
  ...
  template <typename FILLTYPE,
            typename = typename std::enable_if<
                std::is_base_of<cnine::fill_pattern, FILLTYPE>::value,
                FILLTYPE>::type>
  SnPartB(const SnIrrepObj *_irrep, const int n, const FILLTYPE &fill,
          const int _dev = 0)
  ...
  template <typename FILLTYPE,
            typename = typename std::enable_if<
                std::is_base_of<cnine::fill_pattern, FILLTYPE>::value,
                FILLTYPE>::type>
  SnPartB(const SnIrrep &_irrep, const int n, const FILLTYPE &fill,
          const int _dev = 0)
      : rtensor(cnine::dims(1, _irrep.obj->d, n), fill, _dev),
        irrep(_irrep.obj) {}
  ...
  template <typename FILLTYPE,
            typename = typename std::enable_if<
                std::is_base_of<cnine::fill_pattern, FILLTYPE>::value,
                FILLTYPE>::type>
  SnPartB(const IntegerPartition &_lambda, const int n, const FILLTYPE &fill,
          const int _dev = 0)
      : rtensor(
            cnine::dims(
                1, _snbank->get_Sn(_lambda.getn())->get_irrep(_lambda)->d, n),
            fill, _dev),
  ...
  template <typename FILLTYPE,
            typename = typename std::enable_if<
                std::is_base_of<cnine::fill_pattern, FILLTYPE>::value,
                FILLTYPE>::type>
  SnPartB(const int b, const initializer_list<int> list, const int n,
          const FILLTYPE &fill, const int _dev = 0)
  ...
  template <typename FILLTYPE,
            typename = typename std::enable_if<
                std::is_base_of<cnine::fill_pattern, FILLTYPE>::value,
                FILLTYPE>::type>
  SnPartB(const int b, const SnIrrepObj *_irrep, const int n,
          const FILLTYPE &fill, const int _dev = 0)
  ...
  template <typename FILLTYPE,
            typename = typename std::enable_if<
                std::is_base_of<cnine::fill_pattern, FILLTYPE>::value,
                FILLTYPE>::type>
  SnPartB(const int b, const SnIrrep &_irrep, const int n, const FILLTYPE &fill,
          const int _dev = 0)
      : rtensor(cnine::dims(b, _irrep.obj->d, n), fill, _dev),
        irrep(_irrep.obj) {}
  ...
  template <typename FILLTYPE,
            typename = typename std::enable_if<
                std::is_base_of<cnine::fill_pattern, FILLTYPE>::value,
                FILLTYPE>::type>
  SnPartB(const int b, const IntegerPartition &_lambda, const int n,
          const FILLTYPE &fill, const int _dev = 0)
      : rtensor(
            cnine::dims(
                b, _snbank->get_Sn(_lambda.getn())->get_irrep(_lambda)->d, n),
            fill, _dev),
        irrep(_snbank->get_Sn(_lambda.getn())->get_irrep(_lambda)) {}
...
public: // ---- named constructors ----
  static SnPartB raw(const initializer_list<int> list, const int n,
                     const int _dev = 0) {
    return SnPartB(list, n, cnine::fill_raw(), _dev);
  }

  static SnPartB zero(const initializer_list<int> list, const int n,
                      const int _dev = 0) {
    return SnPartB(list, n, cnine::fill_zero(), _dev);
  }

  static SnPartB identity(const initializer_list<int> list, const int n,
                          const int _dev = 0) {
    return SnPartB(list, n, cnine::fill_identity(), _dev);
  }

  static SnPartB gaussian(const initializer_list<int> list, const int n,
                          const int _dev = 0) {
    return SnPartB(list, n, cnine::fill_gaussian(), _dev);
  }

  static SnPartB raw(const IntegerPartition &_lambda, const int n,
                     const int _dev = 0) {
    return SnPartB(_lambda, n, cnine::fill_raw(), _dev);
  }

  static SnPartB zero(const IntegerPartition &_lambda, const int n,
                      const int _dev = 0) {
    return SnPartB(_lambda, n, cnine::fill_zero(), _dev);
  }

  static SnPartB identity(const IntegerPartition &_lambda, const int n,
                          const int _dev = 0) {
    return SnPartB(_lambda, n, cnine::fill_identity(), _dev);
  }

  static SnPartB gaussian(const IntegerPartition &_lambda, const int n,
                          const int _dev = 0) {
  ...
  static SnPartB identity(const SnIrrep &_rho, const int n,
                          const int _dev = 0) {
    return SnPartB(_rho, n, cnine::fill_identity(), _dev);
  }

  static SnPartB gaussian(const SnIrrep &_rho, const int n,
                          const int _dev = 0) {
    return SnPartB(_rho, n, cnine::fill_gaussian(), _dev);
  }

  static SnPartB raw(const int b, const initializer_list<int> list, const int n,
                     const int _dev = 0) {
    return SnPartB(b, list, n, cnine::fill_raw(), _dev);
  }

  static SnPartB zero(const int b, const initializer_list<int> list,
                      const int n, const int _dev = 0) {
    return SnPartB(b, list, n, cnine::fill_zero(), _dev);
  }

  static SnPartB identity(const int b, const initializer_list<int> list,
                          const int n, const int _dev = 0) {
    return SnPartB(b, list, n, cnine::fill_identity(), _dev);
  }

  static SnPartB gaussian(const int b, const initializer_list<int> list,
                          const int n, const int _dev = 0) {
    return SnPartB(b, list, n, cnine::fill_gaussian(), _dev);
  }

  static SnPartB raw(const int b, const IntegerPartition &_lambda, const int n,
                     const int _dev = 0) {
    return SnPartB(b, _lambda, n, cnine::fill_raw(), _dev);
  }

  static SnPartB zero(const int b, const IntegerPartition &_lambda, const int n,
                      const int _dev = 0) {
    return SnPartB(b, _lambda, n, cnine::fill_zero(), _dev);
  }

  static SnPartB identity(const int b, const IntegerPartition &_lambda,
                          const int n, const int _dev = 0) {
    return SnPartB(b, _lambda, n, cnine::fill_identity(), _dev);
  }

  static SnPartB gaussian(const int b, const IntegerPartition &_lambda,
                          const int n, const int _dev = 0) {
    return SnPartB(b, _lambda, n, cnine::fill_gaussian(), _dev);
  }

  static SnPartB raw(const int b, const SnIrrep &_rho, const int n,
                     const int _dev = 0) {
    return SnPartB(b, _rho, n, cnine::fill_raw(), _dev);
  }

  static SnPartB zero(const int b, const SnIrrep &_rho, const int n,
                      const int _dev = 0) {
    return SnPartB(b, _rho, n, cnine::fill_zero(), _dev);
  }

  static SnPartB identity(const int b, const SnIrrep &_rho, const int n,
                          const int _dev = 0) {
    return SnPartB(b, _rho, n, cnine::fill_identity(), _dev);
  }

  static SnPartB gaussian(const int b, const SnIrrep &_rho, const int n,
                          const int _dev = 0) {
...
public
    : // ---- Copying
      // -----------------------------------------------------------------------------------
  SnPartB(const SnPartB &x) : rtensor(x), irrep(x.irrep) {}

  SnPartB(SnPartB &&x) : rtensor(std::move(x)), irrep(x.irrep) {}
...
public
    : // ---- Access
      // -------------------------------------------------------------------------------------
  int getn() const { return irrep->lambda.getn(); }

  SnIrrep get_rho() const { return irrep; }

  IntegerPartition get_lambda() const { return irrep->lambda; }

  IntegerPartition key() const { return irrep->lambda; }

  int getb() const { return dims(0); }

  int getm() const { return dims(2); }

  int getd() const { return dims(1); }
...
public
  ...
  void add_block_to(const int ioffs, const int joffs, rtensor& M, float c=1.0)
  const{ int I=M.dim(0); int J=M.dim(1); for(int i=0; i<I; i++) for(int j=0;
  j<J; j++) M.inc(i,j,get_value(i+ioffs,j+joffs)*c);
  }
