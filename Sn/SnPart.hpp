...
public:
  template <typename FILLTYPE,
            typename = typename std::enable_if<
                ...
                FILLTYPE>::type>
  SnPart(const initializer_list<int> list, const int n, const FILLTYPE &fill,
         const int _dev = 0)
  ...
  template <typename FILLTYPE,
            typename = typename std::enable_if<
                std::is_base_of<cnine::fill_pattern, FILLTYPE>::value,
                FILLTYPE>::type>
  SnPart(const SnIrrepObj *_irrep, const int n, const FILLTYPE &fill,
         const int _dev = 0)
  ...
  template <typename FILLTYPE,
            typename = typename std::enable_if<
                std::is_base_of<cnine::fill_pattern, FILLTYPE>::value,
                FILLTYPE>::type>
  SnPart(const SnIrrep &_irrep, const int n, const FILLTYPE &fill,
         const int _dev = 0)
  ...
  template <typename FILLTYPE,
            typename = typename std::enable_if<
                std::is_base_of<cnine::fill_pattern, FILLTYPE>::value,
                FILLTYPE>::type>
  SnPart(const IntegerPartition &_lambda, const int n, const FILLTYPE &fill,
         const int _dev = 0)
      : rtensor(cnine::dims(
                    _snbank->get_Sn(_lambda.getn())->get_irrep(_lambda)->d, n),
                fill, _dev),
...
public: // ---- named constructors ----
  static SnPart raw(const initializer_list<int> list, const int n,
                    const int _dev = 0) {
    return SnPart(list, n, cnine::fill_raw(), _dev);
  }

  static SnPart zero(const initializer_list<int> list, const int n,
                     const int _dev = 0) {
    return SnPart(list, n, cnine::fill_zero(), _dev);
  }

  static SnPart identity(const initializer_list<int> list, const int n,
                         const int _dev = 0) {
    return SnPart(list, n, cnine::fill_identity(), _dev);
  }

  static SnPart gaussian(const initializer_list<int> list, const int n,
                         const int _dev = 0) {
    return SnPart(list, n, cnine::fill_gaussian(), _dev);
  }

  static SnPart raw(const IntegerPartition &_lambda, const int n,
                    const int _dev = 0) {
    return SnPart(_lambda, n, cnine::fill_raw(), _dev);
  }

  static SnPart zero(const IntegerPartition &_lambda, const int n,
                     const int _dev = 0) {
    return SnPart(_lambda, n, cnine::fill_zero(), _dev);
  }

  static SnPart identity(const IntegerPartition &_lambda, const int n,
                         const int _dev = 0) {
    return SnPart(_lambda, n, cnine::fill_identity(), _dev);
  }

  static SnPart gaussian(const IntegerPartition &_lambda, const int n,
                         const int _dev = 0) {
...
public
    : // ---- Copying
      // -----------------------------------------------------------------------------------
  SnPart(const SnPart &x) : rtensor(x), irrep(x.irrep) {}

  SnPart(SnPart &&x) : rtensor(std::move(x)), irrep(x.irrep) {}
...
public
  ...
  void add_block_to(const int ioffs, const int joffs, rtensor &M,
                    float c = 1.0) const {
    int I = M.dim(0);
    int J = M.dim(1);
    for (int i = 0; i < I; i++)
  ...
  void add_to_block_block(const int ioffs, const int joffs, const rtensor &M,
                          const int io, const int jo, const int I,
                          const int J) {
    for (int i = 0; i < I; i++)
      for (int j = 0; j < J; j++)
