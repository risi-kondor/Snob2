...
public: // conversions
  SnElement(const Permutation &x) : Permutation(x) {}

  SnElement(const int n, const Transposition &x) : Permutation(n, x) {}
...
public: // operations
  ...
  SnElement inverse() const { return SnElement(Permutation::inverse()); }

  SnElement inv() const { return inverse(); }
};
