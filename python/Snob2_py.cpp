...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  pybind11::class_<Permutation>(
      m, "Permutation", "Class to represent a permutation sigma of (1,2,...,n)")
      ...
      .def_static("identity",
                  static_cast<Permutation (*)(int)>(&Permutation::Identity),
                  "Return the identity permutation of n.")

      .def("getn", &Permutation::getn, "Return n.")
      .def("__getitem__",
           static_cast<int (Permutation::*)(const int) const>(
  ...
  pybind11::class_<IntegerPartition>(
      m, "IntegerPartition",
      ...
      .def("__getitem__",
           static_cast<int (IntegerPartition::*)(const int) const>(
               &IntegerPartition::operator()),
  ...
  pybind11::class_<IntegerPartitions>(
      m, "IntegerPartitions",
      ...
      .def("__len__", &IntegerPartitions::size,
           "Return the number of integer partition.")
      .def("at", &IntegerPartitions::operator[],
           "Return the i'th integer partition.")
      //.def("str",&IntegerPartition::str,py::arg("indent")="")
      //.def("__str__",&IntegerPartition::str,py::arg("indent")="")
      /*
      .def("__getitem__",[](const IntegerPartitions& obj, const py::slice&
      slice){ size_t start, stop, step, slicelength; if
      (!slice.compute(obj.size(), &start, &stop, &step, &slicelength)) throw
      py::error_already_set(); auto v=obj(start,stop+1); py::list R; for(auto&
      p:v){ py::object x=py::class_<IntegerPartition>(p); R.append(x);
            //R.append(py::cast(p));
          }
          return R;
        })
  ...
  pybind11::class_<YoungTableau>(m, "YoungTableau")
      ...
      .def(pybind11::init([](const vector<int> v) {
        return std::unique_ptr<YoungTableau>(
            new YoungTableau(IntegerPartition(v)));
      }))
      ...
      .def("__call__",
           static_cast<int (YoungTableau::*)(const int, const int) const>(
               &YoungTableau::at),
           "Return the integer at position (i,j) in the tableau.")
      .def("__getitem__",
           [](const YoungTableau &obj, vector<int> v) {
             assert(v.size() == 2);
             return obj.at(v[0], v[1]);
           })
      .def("__setitem__",
           [](YoungTableau &obj, vector<int> v, int x) {
             assert(v.size() == 2);
             return obj.set_value(v[0], v[1], x);
           })

      //.def("__getitem__",static_cast<int(IntegerPartition::*)(const
      //int)const>(&IntegerPartition::operator()))
  ...
  pybind11::class_<StandardYoungTableaux>(
      m, "StandardYoungTableaux",
      "Class representing standard Young tableaux of a given shape.")
      .def(pybind11::init<const IntegerPartition &>())

      .def("__len__", &StandardYoungTableaux::size,
           "Return the number of standard Young tableaux.")
      .def("__getitem__", &StandardYoungTableaux::operator[], "")

      .def("__str__", &StandardYoungTableaux::str, py::arg("indent") = "",
