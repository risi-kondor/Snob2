...
public:
  ...
  SnCharacter(const IntegerPartition &_lambda)
      : SnClassFunction(*_sncharbank->get_character(_lambda)), lambda(_lambda) {
  } // make it view

public
    : // ---- Conversions
      // -------------------------------------------------------------------------------
  SnCharacter(const SnClassFunction &x) : SnClassFunction(x) {}

  SnCharacter(SnClassFunction &&x) : SnClassFunction(std::move(x)) {}
