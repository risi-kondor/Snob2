...
public:
  template <typename FILLTYPE,
            typename = typename std::enable_if<
                ...
                FILLTYPE>::type>
  SnMultiPart(const int _N, const SnIrrepObj *_irrep, const int _m,
              const FILLTYPE &fill, const int _dev = 0)
      : N(_N), rtensor(cnine::dims(_irrep->d, _N, _m), fill, _dev),
        irrep(_irrep) {}
  ...
  template <typename FILLTYPE,
            typename = typename std::enable_if<
                std::is_base_of<cnine::fill_pattern, FILLTYPE>::value,
                FILLTYPE>::type>
  SnMultiPart(const int _N, const IntegerPartition &_lambda, const int _m,
              const FILLTYPE &fill, const int _dev = 0)
      : N(_N), rtensor(cnine::dims(_snbank->get_irrep(_lambda)->d, _N, _m),
                       fill, _dev),
...
public
    : // ---- Named constructors
      // -------------------------------------------------------------------------
  static SnMultiPart zero(const int _N, const IntegerPartition &_lambda,
                          const int n, const int _dev = 0) {
    return SnMultiPart(_N, _lambda, n, cnine::fill_zero(), _dev);
  }

  static SnMultiPart identity(const int _N, const IntegerPartition &_lambda,
                              const int n, const int _dev = 0) {
    return SnMultiPart(_N, _lambda, n, cnine::fill_identity(), _dev);
  }

  static SnMultiPart gaussian(const int _N, const IntegerPartition &_lambda,
                              const int n, const int _dev = 0) {
...
public
    : // ---- Copying
      // -----------------------------------------------------------------------------------
  SnMultiPart(const SnMultiPart &x) : N(x.N), rtensor(x), irrep(x.irrep) {}
...
public
    : // ---- Conversions
      // --------------------------------------------------------------------------------
  SnMultiPart(const SnFunction &f) : rtensor(f) {
    N = f.N;
    reshape({1, N, 1});
    irrep = _snbank->get_irrep({1});
  }

  SnMultiPart(const SnOverSmFunction &f) : rtensor(f) {
    N = f.getN();
    reshape({1, N, 1});
    irrep = _snbank->get_irrep({1});
  }
...
public
  ...
  cnine::Rtensor3_view view3() const { return RtensorA::view3(); }
...
public
  ...
  void set_block_multi(const int ioffs, const int joffs, const SnMultiPart &M) {
    view3()
        .fuse01()
        .block(ioffs * N, joffs, M.dim(0) * M.dim(1), M.dim(2))
        .set(M.view3().fuse01());
  }

  void add_to_block_multi(const int ioffs, const int joffs,
                          const SnMultiPart &M) {
  ...
  void add_block_to_multi(const int ioffs, const int joffs, SnMultiPart &M,
                          float c = 1.0) const {
    assert(getN() == M.getN());
    int I = M.dim(0);
    int J = M.getm();
    M.view3().fuse01().add(
        view3().fuse01().block(ioffs * N, joffs, M.dim(0) * N, M.dim(2)), c);
    /*
    for(int s=0; s<N; s++)
      for(int i=0; i<I; i++)
        for(int j=0; j<J; j++)
...
public
  ...
  SnMultiPart &apply_inplace(const SnElement &sigma, const int o,
                             const int s = 1) {
    int m = getm();
    reshape({dims(0), N * m});
  ...
  SnMultiPart &apply_inplace(const ContiguousCycle &cyc, const int o,
                             const int s = 1) {
    int m = getm();
    reshape({dims(0), N * m});
  ...
  SnMultiPart &apply_inplace_inv(const ContiguousCycle &cyc, const int o,
                                 const int s = 1) {
    int m = getm();
    reshape({dims(0), N * m});
...
/*
static SnMultiPart zero(const int _N, const initializer_list<int> list, const
int n, const int _dev=0){ return
SnMultiPart(_N,list,n,cnine::fill_zero(),_dev);}

static SnMultiPart identity(const int _N, const initializer_list<int> list,
const int n, const int _dev=0){ return
SnMultiPart(_N,list,n,cnine::fill_identity(),_dev);}

static SnMultiPart gaussian(const int _N, const initializer_list<int> list,
const int n, const int _dev=0){ return
SnMultiPart(_N,list,n,cnine::fill_gaussian(),_dev);}
*/

/*
template<typename FILLTYPE, typename = typename
std::enable_if<std::is_base_of<cnine::fill_pattern, FILLTYPE>::value,
FILLTYPE>::type> SnMultiPart(const int _N, const initializer_list<int> list,
const int _m, const FILLTYPE& fill, const int _dev=0): N(_N),
