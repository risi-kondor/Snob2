...
public:
  SnMultiPart(int _N, const SnIrrep &_irrep, const int _n, const int _dev = 0)
      : rtensor(cnine::dims(_irrep.obj->d, _N * _n), cnine::fill::raw, dev),
        irrep(_irrep.obj), N(_N), n(_n) {}

  SnMultiPart(int _N, const IntegerPartition &_lambda, const int _n,
              const int _dev = 0)
      : rtensor(
            cnine::dims(_snbank->get_Sn(_lambda.getn())->get_irrep(_lambda)->d,
                        _N * _n),
            cnine::fill::raw, dev),
        irrep(_snbank->get_Sn(_lambda.getn())->get_irrep(_lambda)), N(_N),
        n(_n) {}

  template <typename FILLTYPE>
  SnMultiPart(int _N, const SnIrrep &_irrep, const int _n, const FILLTYPE &fill,
              const int _dev = 0)
      : rtensor(cnine::dims(_irrep.obj->d, _N * _n), fill, dev),
        irrep(_irrep.obj), N(_N), n(_n) {}
...
public: // ---- named constructors ----
  static SnMultiPart zero(int N, const IntegerPartition &_lambda, const int n,
                          const int _dev = 0) {
    return SnMultiPart(N, _lambda, n, cnine::fill_zero(), _dev);
  }

  static SnMultiPart identity(int N, const IntegerPartition &_lambda,
                              const int n, const int _dev = 0) {
    return SnMultiPart(N, _lambda, n, cnine::fill_identity(), _dev);
  }

  static SnMultiPart gaussian(int N, const IntegerPartition &_lambda,
                              const int n, const int _dev = 0) {
    return SnMultiPart(N, _lambda, n, cnine::fill_gaussian(), _dev);
  }

  static SnMultiPart zero(int N, const SnIrrep &_rho, const int n,
                          const int _dev = 0) {
    return SnMultiPart(N, _rho, n, cnine::fill_zero(), _dev);
  }

  static SnMultiPart identity(int N, const SnIrrep &_rho, const int n,
                              const int _dev = 0) {
    return SnMultiPart(N, _rho, n, cnine::fill_identity(), _dev);
  }

  static SnMultiPart gaussian(int N, const SnIrrep &_rho, const int n,
                              const int _dev = 0) {
...
public: // ---- Fourier transforms ------------------
  ...
  operator SnPart() const { return SnPart(SnIrrep(irrep), *this); }
