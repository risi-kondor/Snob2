...
class SnCGbank {
public:
  unordered_map<tuple<IntegerPartition, IntegerPartition, IntegerPartition>,
                int>
      coeffs;
  ...
  unordered_map<pair<IntegerPartition, IntegerPartition>, SnCGfactors *>
      factors;
...
public:
  int coeff(const IntegerPartition &_mu1, const IntegerPartition &_mu2,
            const IntegerPartition &_mu3) {
  ...
  const SnType &CGproduct(const IntegerPartition &lambda1,
                          const IntegerPartition &lambda2) {
  ...
  const SnCGfactors &get_CGfactors(const IntegerPartition &lambda1,
                                   const IntegerPartition &lambda2) {
    pair<IntegerPartition, IntegerPartition> lambdas(lambda1, lambda2);
  ...
  void learn(SnCGfactors &R, const IntegerPartition &lamb1,
             const IntegerPartition &lamb2) {
    cout << "---- Learning transformation for " << lamb1 << " * " << lamb2
         << " -> " << CGproduct(lamb1, lamb2) << endl;
    ...
    for (auto p : tau) {
      cout << "  "
           << "Pulling to " << p.first << " with multiplicity " << p.second
           << endl;
      ...
      p.first.foreach_sub([&](const IntegerPartition &mu) {
        ...
        for (int k = 0; k < p.second; k++) {
          ...
          int i = 0;
          while (i < dmu && abs(evals.get_value(i) - target_eval) > 0.001) {
            i++;
          }
          if (i == dmu) {
            cout << "Panic: eigenvalue not found!" << endl;
            exit(-1);
          }
          cout << "Found." << endl;
        ...
        if (first_mu) {
          pullback = (sub[mu].view3().slice1(0)) *
                     (R[mu].block2(0, taken[mu] - p.second, -1, p.second));
          rho.apply(split0(pullback.view2(), 1, d), n - 1);
          first_mu = false;
        } else {
          rtensor M = (sub[mu].view3().slice1(0).transp()) * (pullback.view2());
          rtensor B =
              (R[mu].block2(0, taken[mu] - p.second, -1, p.second).transp()) *
              M;
          cout << "-----> " << B << endl;
          ...
          if (abs(A.get_value(0, offs, 0)) > 0.0001) {
            rtensor C = R[mu].block2(0, taken[mu] - p.second, -1, p.second) *
                        (B.view2().transp() * (1.0 / A.get_value(0, offs, 0)));
  ...
  void add_CGprod(SnVec &R, const SnVec &x, const SnVec &y,
                  const string indent = "") {
    SNOB2_CGBANK_DEBUG(cout << indent << "Multiply(" << x.get_type() << ","
                            << y.get_type() << ") into " << R.get_type()
                            << endl;)
    SnType offs;
    for (auto p : x.parts)
      for (auto q : y.parts) {
  ...
  void accumulate_CGprod(SnVec &R, SnType &offs, const SnPart &x,
                         const SnPart &y, const string indent = "") {
    SNOB2_CGBANK_DEBUG(cout << indent << "Accumulate(" << x.get_lambda() << ","
                            << y.get_lambda() << ") into " << R.get_type()
                            << " with offset " << offs << endl;)
    const int n = x.getn();
    assert(y.getn() == n);
    ...
    for (auto p : get_CGfactors(x.get_lambda(), y.get_lambda()).keyval()) {
      ...
      sub_tilde.parts.insert(mu,
                             new SnPart(SnIrrep(mu), (*sub.parts[mu]) * (*p)));
    }
...
public
  ...
  SnVecB CGproduct(const SnPartB &x, const SnPartB &y) {
    SnVecB R =
        SnVecB::zero(sameb(x, y), CGproduct(x.get_lambda(), y.get_lambda()));
    SnType offs;
    accumulate_CGprod(R, offs, x, y);
    return R;
  }

  void add_CGprod(SnVecB &R, const SnVecB &x, const SnVecB &y,
                  const string indent = "") {
    SNOB2_CGBANK_DEBUG(cout << indent << "Multiply(" << x.get_type() << ","
                            << y.get_type() << ") into " << R.get_type()
                            << endl;)
  ...
  void accumulate_CGprod(SnVecB &R, SnType &offs, const SnPartB &x,
                         const SnPartB &y, const string indent = "") {
    SNOB2_CGBANK_DEBUG(cout << indent << "Accumulate(" << x.get_lambda() << ","
                            << y.get_lambda() << ") into " << R.get_type()
                            << " with offset " << offs << endl;)
...
it=tables.find(lambdas); if(it!=tables.end()) return it->second;
...
/*
rtensor sequester(const IntegerPartition& lambda1, const IntegerPartition&
lambda2){ SnType* tau=get_type(lambda1,lambda2); for(auto& p:*tau){
...
// SnType* CGproductp(const IntegerPartition& lambda1, const IntegerPartition&
// lambda2){ return get_type(lambda1,lambda2);
// }

// SnType get_type(const IntegerPartition& lambda1, const IntegerPartition&
// lambda2){ return SnType(*get_typep(lambda1,lambda2));
// }
