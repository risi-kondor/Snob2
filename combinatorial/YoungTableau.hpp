...
public: // Access
  ...
  vector<int> at(const int r) const { return rows.at(r); }

  int at(const int r, const int c) const { return rows.at(r).at(c); }

  int operator()(const int r, const int c) const { return rows.at(r).at(c); }

  int get_value(const int r, const int c) const { return rows.at(r).at(c); }
...
public:
  ...
  int apply_transp(const int i) { return apply_transp(i, i + 1); }
