...
class SnMultiVec {
public:
  cnine::indexed_mapB<IntegerPartition, SnMultiPart> parts;
...
public:
  ...
  template <typename FILLTYPE,
            typename = typename std::enable_if<
                ...
                FILLTYPE>::type>
  SnMultiVec(const int _N, const SnType &_type, const FILLTYPE &fill,
             const int _dev = 0) {
    for (auto &p : _type)
      parts.insert(p.first, new SnMultiPart(_N, p.first, p.second, fill, _dev));
  }

  SnMultiVec(SnMultiPart *part) { parts.insert(part->get_lambda(), part); }

public
    : // ---- Named constructors
      // -------------------------------------------------------------------------
  static SnMultiVec zero(const int _N, const SnType &_type,
                         const int _dev = 0) {
    return SnMultiVec(_N, _type, cnine::fill::zero, _dev);
  }

  static SnMultiVec identity(const int _N, const SnType &_type,
                             const int _dev = 0) {
    return SnMultiVec(_N, _type, cnine::fill::identity, _dev);
  }

  static SnMultiVec gaussian(const int _N, const SnType &_type,
                             const int _dev = 0) {
...
public
    : // ---- Copying
      // ------------------------------------------------------------------------------------
  SnMultiVec(const SnMultiVec &x) { parts = x.parts; }

  SnMultiVec(SnMultiVec &&x) { parts = std::move(x.parts); }
...
public
    : // ---- Conversions
      // --------------------------------------------------------------------------------
  SnMultiVec(const SnFunction &f) { parts.push_back(new SnMultiPart(f)); }

  SnMultiVec(const SnOverSmFunction &f) { parts.push_back(new SnMultiPart(f)); }
...
public
  ...
  */
  ...
  SnMultiVec &apply_inplace_inv(const ContiguousCycle &cyc, const int o,
                                const int s = 1) {
    for (auto p : parts)
