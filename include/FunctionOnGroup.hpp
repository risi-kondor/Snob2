...
public
  ...
  FunctionOnGroup(const GROUP *_G, const FILLTYPE &dummy, const int _dev = 0)
      : TENSOR(Gdims({_G->size()}), dummy, _dev), G(_G), N(_G->size()) {}

public:
  FunctionOnGroup(const FunctionOnGroup &x) : TENSOR(x), G(x.G), N(x.N) {}

public: // Access
  int size() const { return N; }

  SCALAR operator()(const int i) const { return TENSOR::value(i); }
