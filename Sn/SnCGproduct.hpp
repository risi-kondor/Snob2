...
namespace Snob2{
  ...
  void SnVec_addCGproduct(SnVec& R, SnType& offs, const SnPart& x, const SnPart& y){
    const int n=x.getn();
    assert(y.getn()==n);
	...
	if(ncols==0) ncols=M.dims(1);
	target.view2D_block(roffs,offs[lamb],nrows,ncols).
	  add_matmul_AA(source.view2D_block(0,sub_offs[mu],nrows,ncols),M.view2D());
	roffs+=nrows;
	sub_offs[mu]+=ncols;
      }
      offs[lamb]+=ncols;
